...
const Amaranta = lazy(() => import('./pages/Amaranta').then((module) => ({ default: module.Amaranta })))
...
export function App() {
  ...
  const isAmaranta = path === AMARANTA_ROUTE || path === '/amaranta'
  const page = useMemo(() => {
    if (path === AMARANTA_ROUTE || path === '/amaranta') return <Amaranta />
  ...
  return (
    <>
      <Seo path={path} />
      {!isAmaranta && <Header path={path} go={go} />}
      <main>
